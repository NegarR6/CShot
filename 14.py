...
class Shoot:
    ...
    def shooting(self, keys, up_key, down_key, left_key, right_key, shoot_key, time, bullets, frozen_until):
        ...
        if current_time < frozen_until:  # اگر هنوز فریز است
            return  # اجازه شلیک نده
        else:
            if keys[up_key]:
...
class Game:
    def __init__(self):
        ...
        self.player1_frozen_until = 0  # زمان پایان فریز بازیکن ۱
        self.player2_frozen_until = 0  # زمان پایان فریز بازیکن ۲
    ...
    def display_end_screen(self, score1, score2, player1_name, player2_name):
        ...
        # تأخیر 7 ثانیه‌ای قبل از بسته شدن بازی
    ...
    def run(self):
        ...
        while self.running:
            ...
            shoot1.shooting(keys, pygame.K_w, pygame.K_s, pygame.K_a, pygame.K_d, pygame.K_SPACE, time1, self.player1_bullets, self.player1_frozen_until)
            if keys[pygame.K_SPACE] and not self.space_pressed:
                self.player1_bullets -= 1
                self.space_pressed = True

            shoot2.shooting(keys, pygame.K_UP, pygame.K_DOWN, pygame.K_LEFT, pygame.K_RIGHT, pygame.K_RETURN, time2, self.player2_bullets, self.player2_frozen_until)
            ...
            for shot in shoot1.shotsl:
                ...
                if target_frozen.check_collision(shot):
                    self.score1, self.number_of_shots1_p1 = point1.giving_points(target_frozen, shoot1, self.score1, self.number_of_shots1_p1, shot, targets, point1)
                    self.player2_frozen_until = pygame.time.get_ticks() + 5000  # ۵ ثانیه فریز
            ...
            for shot in shoot2.shotsl:
                ...
                if target_frozen.check_collision(shot):
                    self.score2, self.number_of_shots1_p2 = point2.giving_points(target_frozen, shoot2, self.score2, self.number_of_shots1_p2, shot, targets, point2)
                    self.player1_frozen_until = pygame.time.get_ticks() + 5000  # ۵ ثانیه فریز
